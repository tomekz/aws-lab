...
* [X] Set up Kafka on the EC2 cluster:
  - [X] Install Kafka on both EC2 instances within the cluster. 
       ...
       - [X] To allow Kafka and ZooKeeper to persist files on the host filesystem, we need to map two directories from the host to the Docker containers:
             ```
               volumes:
                - ./data/kafka:/bitnami/kafka
                - ./data/zookeeper:/bitnami/zookeeper
             ```
        - [X] change the ownerships of the two directories, granting the container processes the permissions to read and write.
              ```
